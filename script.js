...
export function startGame(sanityOn) {
    ...
    function mainMenu() {
        alert('Return to main menu – not implemented.');
    }
    ...
    function updateStaminaUI() {
        ...
        if (sanity < 30) {
            bar.classList.add('fight-or-flight');
        } else {
            bar.classList.remove('fight-or-flight');
        }
    }
    ...
    // ─── Init ──────────────────────────────────────────
    // Wait a moment to ensure the container has size
    setTimeout(() => {
        generateLevel(0);
    ...
    function animate(time) {
        ...
        // Skip if container has zero size (prevents framebuffer errors)
        ...
        updateStaminaUI();
        document.getElementById('staminaContainer').style.opacity = (isSprinting && isMoving && onGround) ? '0.9' : '0';
